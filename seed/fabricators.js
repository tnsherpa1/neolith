var Fabricator = require('../models/fabricator');
...
Fabricator.remove({}, function(err) {
  if (err) {
    console.log(err);
  } else {
    Fabricator.create(fabricators, function(err, fabricators) {
      if(err) {
        console.log(err);
      }
        else {
          console.log("created :", fabricators);
